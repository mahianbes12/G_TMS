...
const TaskListForm = ({ setTasklists, showSideTasklistForm }) => {
  ...
  const onSubmit = async ({ name }) => {
    ...
    await axios.post(`http://localhost:3000/project/${projectId}/tasklist`, { name , projectId });

    const res = await axios.get(`http://localhost:3000/project/${projectId}/tasklists`);
