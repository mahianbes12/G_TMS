...
const AddMemberForm = ({ teamId, clickClose, open, setTeamUsers }) => {
  ...
  const onSubmit = async ({ userId }) => {
    try {
      await axios.post(`http://localhost:3000/team/${teamId}/user/${userId}`);
      const res = await axios.get(`http://localhost:3000/team/${teamId}`);
    ...
    // const res = await axios.get(`/project/${projectId}/tasklists`);
  };
